...
namespace Sefacan.Framework.Helpers
{
    public static class WebHelper
    {
        public static UserCookieModel CurrentUser
        {
            get
            {
                var user = new UserCookieModel();
                if (CookieHelper.Exists(CookieConstant.CURRENT_USER))
                    user = CookieHelper.Get(CookieConstant.CURRENT_USER).FromBase64().FromJsonString<UserCookieModel>();
                else
                    user = null;
        ...
        public static bool IsAuthenticated
        {
            get
            {
                try
                {
                    if (CookieHelper.Exists(CookieConstant.CURRENT_USER))
                    {
                        var user = CookieHelper.Get(CookieConstant.CURRENT_USER).FromBase64().FromJsonString<UserCookieModel>();
                        return user != null;
                    }
                }
                catch { }
        ...
        {
            if (IsRequestAvailable && SessionState != null)
